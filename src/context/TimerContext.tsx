...
//   const deleteTimer = (id: string) => saveTimers(timers.filter(t => t.id !== id));
//   const startTimer = (id: string) => saveTimers(timers.map(t => t.id === id ? { ...t, status: 'running', remainingTime: t.duration } : t));
//   const pauseTimer = (id: string) => saveTimers(timers.map(t => t.id === id && t.status === 'running' ? { ...t, status: 'paused' } : t));
//   const resetTimer = (id: string) => saveTimers(timers.map(t => t.id === id ? { ...t, status: 'paused', remainingTime: t.duration } : t));
//   const completeTimer = (id: string) => {
//     const updated = timers.map(t => t.id === id ? { ...t, status: 'completed', remainingTime: 0 } : t);
//     saveTimers(updated);
//     const completed = timers.find(t => t.id === id);
//     if (completed) {
//       saveHistory([...history, { ...completed, completedAt: new Date() }]);
//       Alert.alert('Congratulations!', `${completed.name} completed!`);
//     }
//   };

//   return (
//     <TimerContext.Provider value={{ timers, history, addTimer, deleteTimer, startTimer, pauseTimer, resetTimer, completeTimer }}>
//       {children}
